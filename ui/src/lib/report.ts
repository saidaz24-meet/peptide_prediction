...
export function exportShortlistPDF(
  ...
  const summaryRows = [
    ...
    ["Mean FF-Helix %", fmtPct(stats.meanFFHelixPercent)],
  ...
  pdf.setTextColor(100);
  pdf.text(
    `Weights — H: ${weights.wH}  Charge: ${weights.wCharge}  μH: ${weights.wMuH}  Helix: ${weights.wHelix}  |  FF-Helix threshold: ${ffHelixThreshold}%`,
    MARGIN, y + LINE_H,
  );
  pdf.setTextColor(0);
  y += LINE_H + 10;
  ...
  const notes = [
    ...
    "FF-Helix %: Fraction of residues in fibril-forming helical segments (context-free, intrinsic propensity).",
    "S4PRED Helix %: Neural network secondary structure prediction (context-dependent).",
    "TANGO SSW: Structural Switch prediction from TANGO aggregation analysis.",
    "Ranking Score = wH×H + wCharge×|Charge| + wμH×μH + wHelix×(FF-Helix≥threshold ? 1 : 0).",
    "",
