...
interface ThresholdConfigPanelProps {
  ...
  customThresholds: { muHCutoff: number; hydroCutoff: number; aggThreshold: number; dangerousThreshold?: number; percentOfLengthCutoff?: number; minSswResidues?: number };
  onCustomChange: (t: { muHCutoff: number; hydroCutoff: number; aggThreshold: number; dangerousThreshold?: number; percentOfLengthCutoff?: number; minSswResidues?: number }) => void;
  variant?: 'details' | 'inline';
}

function ThresholdFields({ thresholdMode, onModeChange, customThresholds, onCustomChange }: Omit<ThresholdConfigPanelProps, 'variant'>) {
  return (
    <div className="space-y-4">
      <div>
        <Label htmlFor="threshold-mode">Threshold Mode</Label>
        <Select value={thresholdMode} onValueChange={(v: 'default' | 'recommended' | 'custom') => onModeChange(v)}>
      ...
      {thresholdMode === 'custom' && (
        ...
        <div className="flex items-start gap-2 text-xs text-amber-800 bg-amber-50 border border-amber-200 rounded-md p-2.5">
          ...
          <span>
            Custom thresholds override Peleg's rigorously validated reference values. Changes may affect scientific accuracy of FF classification.
          </span>
        </div>
        <div className="grid grid-cols-3 gap-4">
          <div>
            ...
            <input
              ...
              onChange={(e) => onCustomChange({ ...customThresholds, muHCutoff: parseFloat(e.target.value) || 0 })}
            />
          </div>
          <div>
            ...
            <input
              ...
              onChange={(e) => onCustomChange({ ...customThresholds, hydroCutoff: parseFloat(e.target.value) || 0 })}
            />
          </div>
          <div>
            ...
            <input
              ...
              onChange={(e) => onCustomChange({ ...customThresholds, aggThreshold: parseFloat(e.target.value) || 5 })}
            />
          </div>
          <div>
            ...
            <input
              ...
              onChange={(e) => onCustomChange({ ...customThresholds, dangerousThreshold: parseFloat(e.target.value) || 25 })}
            />
          </div>
          <div>
            ...
            <input
              ...
              onChange={(e) => onCustomChange({ ...customThresholds, percentOfLengthCutoff: parseFloat(e.target.value) || 20 })}
            />
          </div>
          <div>
            ...
            <input
              ...
              onChange={(e) => onCustomChange({ ...customThresholds, minSswResidues: parseInt(e.target.value) || 3 })}
            />
          </div>
        </div>
        </div>
