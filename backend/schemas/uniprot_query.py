...
class UniProtQueryParseResponse(BaseModel):
    ...
    keyword: Optional[str] = Field(
        None, description="Extracted keyword if mode is keyword or keyword_organism"
    )
    organism_id: Optional[str] = Field(
        None, description="Extracted organism ID if mode is organism or keyword_organism"
    )
    normalized_query: str = Field(..., description="Normalized version of input query")
    api_query_string: str = Field(
        ..., description="Final UniProt API query string that will be executed"
    )
