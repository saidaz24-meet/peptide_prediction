...
from services.uniprot_parser import QueryMode, parse_uniprot_query
...
def test_organism_id_with_keyword():
    ...
    assert result.api_query_string == "amyloid AND organism_id:9606"
...
def test_pure_numeric_not_keyword():
    ...
    assert extract_keyword("amyloid 9606") == "amyloid", (
        "Should extract 'amyloid' from 'amyloid 9606'"
    )

    # After removing organism_id pattern, pure numeric should return None
    assert extract_keyword("organism_id:9606") is None, (
        "After removing organism_id pattern, should return None"
    )
...
def test_keyword_parsing():
    ...
    assert result.keyword == "amyloid"
    assert result.api_query_string == "amyloid"
