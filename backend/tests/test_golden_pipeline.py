...
def test_single_sequence_validation():
    ...
    try:
        ...
        raise AssertionError("Should have raised HTTPException for empty sequence")
    except Exception as e:
    ...
    try:
        ...
        raise AssertionError("Should have raised HTTPException for whitespace-only sequence")
    except Exception as e:
        assert "empty" in str(e).lower() or "required" in str(e).lower(), \
            f"Expected error about empty sequence, got: {e}"
...
def test_ff_helix_percent_range():
    ...
    for seq, _expected_range in test_cases:
...
def test_tango_entry_aligned_assignment():
    ...
    entry_to_score = {}
    entry_to_diff = {}
    for _idx, row in df_shuffled.iterrows():
        entry = row["Entry"]
...
def test_ssw_percent_matches_table():
    ...
    print("  ✅ All rows have canonical sswPrediction field with valid -1/0/1 values")
